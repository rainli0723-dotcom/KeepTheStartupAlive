...
export default async function EnterprisePage() {
  ...
  return (
    <AppShell>
      <PageHeader
        title="企业空间"
        description="管理企业账号、成员权限、沙盘工作区和关键操作审计。登录后，数据会绑定到当前企业空间。"
        action={
          auth ? (
            ...
            <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
              <div>
                ...
                <p className="mt-2 text-sm text-[var(--muted)]">
                  套餐：{tenant.plan} · 状态：{tenant.status}
                  {!auth ? " · 当前为未登录演示空间" : ""}
                </p>
              </div>
          ...
          <Panel className="p-5">
            <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
              <div>
                ...
                <p className="mt-1 text-sm text-[var(--muted)]">支持管理员、编辑者、只读三类基础权限。</p>
              </div>
            ...
            <Panel className="p-5">
              ...
              <div className="mt-4 space-y-3">
                ...
                {tenant.workspaces.length === 0 ? (
                  <p className="text-sm text-[var(--muted)]">还没有工作区，可以从“开始模拟”创建。</p>
                ) : null}
              </div>
            </Panel>

            <Panel className="p-5">
              <h2 className="text-lg font-semibold text-white">操作审计</h2>
              <div className="mt-4 space-y-3">
